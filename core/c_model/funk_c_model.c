...
void _dereference(struct tnode * dst, struct tpool * pool,uint32_t start,  int idx, int current_depth, const char * func, int line);
/*
...
void _funk_set_node_dimension(struct tnode * n, uint32_t idx,
   uint32_t val, const char * caller, int line){
     TRACE("start");
  if (idx > DIM_COUNT(n)){
    printf("-E- %s Invalid dimension %d from %s +%d\n",__FUNCTION__,idx, caller,line);
    exit(1);
  }
...
struct tdata * get_node(struct tnode * n, uint32_t i, const char * caller, int line, int check ){
  ...
  if (check && data->wrap_creation < n->pool->wrap_count && n->start <= n->pool->tail){
    printf("-E- %s:%d get_node: trying to access overwritten position %d:%d in '%s':%d\n",
    caller, line,
    idx, data->wrap_creation,
    POOL_STR(n->pool),n->pool->wrap_count);
    printf("%d : %d\n", funk_global_memory_pool.wrap_count, funk_functions_memory_pool.wrap_count);
    printf("Trying to access %s[%d:%d] len:%d, pool_tail = %d/%d\n", POOL_STR(n->pool),
      n->start, (n->start + n->len)%FUNK_MAX_POOL_SIZE, n->len, n->pool->tail, FUNK_MAX_POOL_SIZE);

    printf("wrap_creation: ");
    for (int i = 0; i < n->len; i++){
      printf("%s %d ", POOL_STR(n->pool), n->pool->data[(n->start + i) % FUNK_MAX_POOL_SIZE].wrap_creation);
    }

    printf("wrap count: ");
    for (int i = 0; i < n->len+40; i++){
      printf("%s%d: %d \n",POOL_STR(n->pool), (n->start + i-20 ) % FUNK_MAX_POOL_SIZE, n->pool->data[(n->start + i-20 ) % FUNK_MAX_POOL_SIZE].wrap_creation);
    }

    printf("wrap count: ");
    for (int i = 0; i < n->len+40; i++){
      printf("%s%d: %d \n","fpool", (n->start + i-20 ) % FUNK_MAX_POOL_SIZE, funk_functions_memory_pool.data[(n->start + i-20 ) % FUNK_MAX_POOL_SIZE].wrap_creation);
    }
...
TRACE("start");
  ...
  n->pool->data[(n->start + i) % FUNK_MAX_POOL_SIZE].wrap_creation = n->pool->wrap_count;
  //n->wrap_creation = d;
...
void funk_increment_pool_tail(struct tpool * pool, const char * function, int line){
  TRACE("start");
...
void funk_print_node_info(struct tnode * n){
  ...
  printf("len %d\n", LEN(n));
  printf("%s[%d :%d] %d-d\n", POOL_STR(n->pool), n->start, (n->start + LEN(n)) % FUNK_MAX_POOL_SIZE, DIM_COUNT(n) );
  printf("types: ");
  ...
  printf("\nglobal tail: %d:%d\n", funk_global_memory_pool.tail, funk_global_memory_pool.wrap_count);
  printf("function tail: %d:%d\n", funk_functions_memory_pool.tail, funk_functions_memory_pool.wrap_count);
  printf("\n\n");
...
void funk_deep_copy_node(struct tnode * dst, struct tnode * src){
  ...
  funk_create_node(dst, len, get_pool_enum(src->pool), type_int,
    DIM_COUNT(src) , NULL);
...
void funk_roll(struct tnode * dst, struct tnode * src, struct tnode * deltas, uint32_t delta_count ){
  TRACE("start");
...
void funk_sum_list(struct tnode * dst, struct tnode * src){
  ...
  funk_create_node(dst, 1, get_pool_enum(src->pool), type_int, 0, (void*)&total);
...
void _dereference(struct tnode * dst, struct tpool * pool,uint32_t start,  int idx, int current_depth, const char * function, int line){
  TRACE("start");
  ...
  if (type ==  type_pointer_to_pool_entry){
    ...
    _dereference(dst, pool, pool->data[(start + idx)% FUNK_MAX_POOL_SIZE].data.i, 0, current_depth-1, function, line);
    ...
    printf("-E- %s unknown type %d from %s +%d %s\n",
     __FUNCTION__, type, function, line, POOL_STR(pool));
    exit(1);
  }
...
void funk_print_pool(enum pool_types pool_type, int begin, int len){
  ...
  printf("tail @: %d\n", pool->tail);
   for (int i = begin; i < begin + len; i++){
     ...
     printf("%d(%d):", i, pool->data[i].wrap_creation); funk_print_scalar_element(pool->data[i]); printf("  ");
     if (i >0 && (i + 1) % 7 == 0)
       printf("\n" );
   }
   printf("\n" );
...
void funk_arith_op_rr(struct tnode * node_r, int32_t r_offset,
      ...
      if (IS_PTR(node_a,a_offset)){

        //_flatten_node_pointer_list_to_matrix(&tmp_a, node_a);
        DEREF(&tmp_a,node_a,a_offset);
        node_a = &tmp_a;
      }

      if (IS_PTR(node_b,b_offset)){

        //_flatten_node_pointer_list_to_matrix(&tmp_b, node_b);
        DEREF(&tmp_b,node_b,b_offset);
        node_b = &tmp_b;
...
void funk_print_node(struct tnode * n){
  ...
  uint32_t len = (DIM_COUNT(n) == 2) ? DIM(n,0) : LEN(n);
...
uint32_t _funk_set_node_dimensions(struct tnode * src, enum pool_types pool, uint32_t dim,
                                    uint32_t * dimensions, uint32_t dim_count, uint32_t offset){
  TRACE("start");
  struct tnode new_column;
  //printf("%p\n",src);
  // funk_print_node_info(src);
  ...
  for (uint32_t j = 0; j < n; j++){
    ...
    if (dim+1 < dim_count){
      ...
      DATA(&new_column, j)->data.i = _funk_set_node_dimensions(src, pool, dim+1,
        dimensions, dim_count, offset + j*dimensions[dim+1]);
    } else {

      DATA(&new_column, j)->type = DATA(src, offset + j)->type;
      DATA(&new_column, j)->data.i = DATA(src, offset + j)->data.i;
      //printf("er[%d] %d %d\n",offset+j, DATA(src, offset + j)->data.i,   DATA(&new_column, j)->type);
    }
  }
...
void _extract_tnode_from_pool(struct tnode  * dst, uint32_t start, struct tpool * pool ){
  TRACE("start");
...
void funk_set_node_dimensions(struct tnode  * dst, uint32_t * dimensions, uint32_t dim_count){
  TRACE("start");
  if (dimensions == NULL)
    return;
  ...
  _extract_tnode_from_pool(dst, _funk_set_node_dimensions(dst, pool, 0, dimensions, dim_count, 0), dst->pool);
...
void funk_alloc_tnode_array_from_range_regs(struct tnode  * dst,
  ...
  void funk_set_tnode_array_elementEX(struct tnode  * tnode_list,
    struct tnode  * iterator_reg, struct tnode  * value_reg, uint32_t len){
      TRACE("start");
